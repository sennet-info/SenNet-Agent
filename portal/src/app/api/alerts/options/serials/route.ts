...
export async function GET(request: NextRequest) {
  try {
    ...
    const upstream = await fetch(`${AGENT_BASE_URL.replace(/\/$/, "")}/v1/discovery/serials?tenant=${encodeURIComponent(tenant)}&client=${encodeURIComponent(client)}&site=${encodeURIComponent(site)}`, {
      headers: { Authorization: auth },
      cache: "no-store",
    });
    const data = await upstream.json().catch(() => ({}));
    if (!upstream.ok) return Response.json({ detail: data?.detail ?? "Error discovery serials" }, { status: upstream.status });
