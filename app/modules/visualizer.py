...
def _style(fig, axes):
    for ax in (axes if hasattr(axes,"__iter__") else [axes]):
        ...
        ax.spines["left"].set_color(THEME["border"]); ax.spines["bottom"].set_color(THEME["border"])
...
class Visualizer:
    ...
    @staticmethod
    def create_hourly_profile(data, color=None, unit="kW", sensor_col="", brand_color=None):
        try:
            if not data: return None
            ...
            hours = list(range(24)); vals = [clean.get(h,0) for h in hours]
            fig, ax = plt.subplots(figsize=(6.5,2.7))
            _style(fig,ax)
            x = np.array(hours,dtype=float); y = np.array(vals,dtype=float)
            ...
            ax.set_title("Perfil Diario Promedio", fontsize=9, fontweight="bold", color=THEME["text"], pad=7)
            plt.tight_layout(pad=0.8)
            return _save(fig)
        except: return None
    ...
    def create_line_chart(data, color=None, title_chart="", unit="Valor",
        ...
        try:
            if not data: return None
            pal = _detect_palette(sensor_col, unit, brand_color=None)
            main_c, light_c = pal["main"], pal["light"]
            sorted_items = sorted(data.items())
            x_labs, y_vals = [], []
            for k,v in sorted_items:
            ...
            ax.plot(x, y_vals, color=main_c, linewidth=2, marker="o", markersize=3.5, zorder=3)
    ...
    def create_gauge(value, vmin, vmax, unit="", label="", sensor_col=""):
        try:
            ...
            def arc(t1,t2,c):
                ax.plot(np.linspace(t1,t2,200),[1]*200,color=c,linewidth=10,solid_capstyle="butt",zorder=2)
    ...
    def create_summary_table(rows):
        """
        Tabla resumen con columna Período (si hay múltiples) y tendencia con %.
        """
        try:
            if not rows: return None
            periods = [r.get("period","") for r in rows]
            multi   = len(set(p for p in periods if p)) > 1

            if multi:
                cols = ["Período","Dispositivo","Tipo","Total / Promedio","Coste / Info","Tendencia"]
            else:
            ...
            for i,r in enumerate(rows):
                ...
                if multi:
                    period = r.get("period","")
                    if " - " in period: period = period.split(" - ",1)[1]
                    row = [period, device, r.get("type",""),
                           r.get("main_value","—"), r.get("secondary_value","—"), tcell]
                else:
                    row = [device, r.get("type",""),
                           r.get("main_value","—"), r.get("secondary_value","—"), tcell]

                cell_data.append(row)
                cell_colors.append([THEME["bg2"] if i%2==0 else THEME["bg"]]*len(cols))
            ...
            for (ri,ci),cell in tbl.get_celld().items():
                cell.set_edgecolor(THEME["border"]); cell.set_linewidth(0.4)

            last = len(cols)-1
            for i in range(1,n+1):
                c = {"+":"#B71C1C","-":"#2E7D32","=":"#6C757D"}.get(trend_info[i-1],"#6C757D")
                tbl[i,last].set_text_props(color=c,fontweight="bold",fontsize=7)
